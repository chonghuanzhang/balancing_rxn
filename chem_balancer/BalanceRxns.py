...
def getspecdat_rxn(rxnsmiles,reagents=[],solvents=[]):
    ...
    '''
    try:
        rgt = rxnsmiles[rxnsmiles.find('>')+1:rxnsmiles.rfind('>')]
        if rgt:
            rxnsmiles = rxnsmiles[0:rxnsmiles.find('>')+1] + rxnsmiles[rxnsmiles.rfind('>')::]
            rgts = rgt.split('.')
            rgts = [Chem.MolToSmiles(molfromsmiles(rt)) for rt in rgts]
            reagents = Counter(rgts)
...
def getspecdat(
   ...
   Args:
        ...
        DB (Union[pd.DataFrame, sqlite3.Connection]): Reference database or SQL connection
...
def addspeciesdata(analoguerxns,substancesource,includesolv=False,ncpus=16,restart=True,SQL=False,
    ...
    if not analoguerxns.empty:
        ...
        # Rdata, Pdata, Rgtdata, Solvdata=analoguerxnsdis_.apply(getspecdat)
